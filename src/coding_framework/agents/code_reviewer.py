...
class CodeReview(BaseModel):
    ...
    performance_notes: list[str] = Field(
        default_factory=list, description="Performance observations"
    )
...
Be constructive, specific, and provide examples where helpful."""
    ...
    def _extract_code_and_context(self, request: str) -> dict[str, str]:
        ...
        if code_matches:
            ...
            context = re.sub(
                code_block_pattern, "[CODE_BLOCK_REMOVED]", request, flags=re.DOTALL
            ).strip()
        else:
    ...
    def _build_review_request(
        ...
        if include_suggestions:
            request_parts.append(
                "**Requirements:** Include specific improvement suggestions with examples where helpful."
            )
            ...
            for pattern in patterns:
                ...
                if match:
                    ...
                    items = [
                        ...
                        if item.strip()
                        and (item.strip().startswith("-") or item.strip().startswith("*"))
                    ]
                    break
    ...
    def _categorize_issue(self, issue_description: str) -> str:
        ...
        if any(
            ...
            for word in ["security", "vulnerability", "injection", "validation"]
        ):
            return "security"
        elif any(
            word in description_lower for word in ["performance", "efficiency", "slow", "optimize"]
        ):
            return "performance"
        elif any(
            ...
            word in description_lower
            for word in ["maintainability", "readable", "complex", "structure"]
        ):
